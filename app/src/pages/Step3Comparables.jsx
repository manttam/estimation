...
export default function Step3Comparables() {
  ...
  /* État DVF live : transactions, count total, état de chargement.
   * { loading, transactions:[], count, error } */
  const [dvfLive, setDvfLive] = useState({ loading: false, transactions: [], count: 0, error: null });
  ...
  /* ─── Fetch DVF live (citycode présent) → peuple "others" avec transactions réelles ───
   * Pattern identique à Step2 : appel /api/dvf?citycode=XXX&type=YYY (proxy Vercel),
   * transformation transactions → shape comparable card OTHERS, override de l'état.
   *
   * Retombe gracieusement sur les mocks si :
   *   - pas de citycode (mode démo, fallback Lyon 3)
   *   - fetch échoue
  ...
  useEffect(() => {
    ...
    if (!citycode) {
      console.log('[DVF Step3] pas de citycode → mode démo (mocks Lyon 3)');
      return;
    }
    let cancelled = false;
    ...
    console.log('[DVF Step3] fetch', url);
    setDvfLive((s) => ({ ...s, loading: true, error: null }));
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return;
        if (!data || !data.ok) {
          console.warn('[DVF Step3] API NOK', data?.error || 'unknown');
          setDvfLive({ loading: false, transactions: [], count: 0, error: data?.error || 'no data' });
          return;
        }
        const txs = Array.isArray(data.transactions) ? data.transactions : [];
        console.log('[DVF Step3] ✓', txs.length, 'transactions reçues sur', data.count || '—', 'total');
        setDvfLive({
          loading: false,
          transactions: txs,
        ...
        // Transforme transactions → cards OTHERS et override l'état.
        const liveOthers = txs
          .map((tx, i) => dvfTxToOther(tx, i, targetCoords))
          .filter((c) => c && c.coords); // exclut les tx sans géolocalisation
        // Tri par distance croissante (les plus proches d'abord)
        liveOthers.sort((a, b) => {
          const da = haversineMeters(targetCoords, a.coords);
          const db = haversineMeters(targetCoords, b.coords);
          return (da ?? 1e9) - (db ?? 1e9);
        });
        setOthers(liveOthers);
      })
      .catch((err) => {
        if (cancelled) return;
        console.warn('[DVF Step3] fetch error', err.message);
        setDvfLive({ loading: false, transactions: [], count: 0, error: err.message });
      });
  ...
  const computeFilteredCount = () => {
    // Mode live : count réel de transactions DVF sur la commune (capé pour cohérence UI)
    if (hasRealLocation && dvfLive && dvfLive.count > 0) {
      // Approximation simple : on prend le total transactions et on applique les
      // filtres de sources (DVF / Ideeri / etc.). En l'absence de Ideeri/Encours/Portail
      // live, on n'affiche que les DVF (les autres sources restent à 0 en live).
      const sourcesOn = [sourceDvf, sourceIdeeri, sourceEncours, sourcePortail].filter(Boolean).length;
      // Si la source DVF est désactivée, count = 0
      if (!sourceDvf) return 0;
      // Sinon le count = nombre de transactions DVF (les autres sources sont absentes en live)
